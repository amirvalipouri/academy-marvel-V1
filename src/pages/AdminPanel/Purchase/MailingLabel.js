...
export default function MailingLabel({ info = {} }) {
  ...
  return (
    <div className="mt-5 d-none d-print-block">
      <h4 className="w-100 text-center mb-2"> برچسب پستی</h4>
      <Row
      ...
      >
        {items.map((e, i) => (
          <Col
            key={i}
            xs={!!e.length ? "7" : "5"}
            className="border border-light-gray py-2 px-3 rounded"
          >
            {e.map((e, index) => (
              <p key={index}>
      ...
      <h4 className="w-100 text-center mt-4 mb-2">فاکتور</h4>
      <Row
        style={{ border: "1px dashed" }}
        className="align-items-stretch row-gap-0 border-dark p-1 rounded"
      >
