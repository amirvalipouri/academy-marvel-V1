...
export default function Modal({
  ...
  return (
    <div
      ref={modal}
      className={`Modal text-center position-fixed overflow-hidden p-2 transition-500  ${className}`}
    >
      <button
        type="button"
        className="hide-btn"
      ...
      >
        <div className="header w-100 d-flex flex-center text-center">
          <div
            className={`title w-100 border-bottom border-light-gray p-2 fs-6 text-${type}  ${titleClassName} `}
          >
            {title}
          </div>
        </div>
